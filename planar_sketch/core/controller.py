...
from ..ui.items import TextMarker, PointItem, LinkItem, AngleItem, CoincideItem, PointLineItem
...
class SketchController:
    def __init__(self, scene: QGraphicsScene, win: "MainWindow"):
        ...
        self.output: Dict[str, Any] = {"enabled": False, "pivot": None, "tip": None}
    ...
    def solve_constraints(self, iters: int = 60, drag_pid: Optional[int] = None):
        ...
        elif driver_type == "vector" and driver_pivot is not None and driver_tip is not None:
    ...
    def cmd_add_point(self, x: float, y: float):
        ...
        class AddPoint(Command):
            name = "Add Point"
            def do(self_):
                ctrl._create_point(pid, x, y, fixed=False, hidden=False)
    ...
    def load_dict(self, data: Dict[str, Any], clear_undo: bool = True):
        ...
        for p in pts:
            ...
            self._create_point(pid, float(p.get("x", 0.0)), float(p.get("y", 0.0)),
                               bool(p.get("fixed", False)), bool(p.get("hidden", False)))
            if pid in self.points:
